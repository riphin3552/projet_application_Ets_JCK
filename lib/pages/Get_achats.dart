...
class GetAchats extends StatefulWidget {
  //const GetAchats({super.key});
  final String nomUtilisateur;

  const GetAchats({Key? key, required this.nomUtilisateur}) : super(key: key);
  

  @override
  State<GetAchats> createState() {
    return _GetAchatsState();
  }
}
class _GetAchatsState extends State<GetAchats> {
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      _onSearchAchats(_searchController.text);
...
// Méthode de recherche par nom planteur
void _onSearchAchats(String query) async {
  String query = _searchController.text.toLowerCase();
  setState(() {
    _filteredAchats = _allAchats.where((achat) {
      final nomPlanteur = achat['nomAgriculteur'].toString().toLowerCase();
      return nomPlanteur.contains(query);
    }).toList();
  });
}
...
List<dynamic> achatsFiltres() {
  ...
  // 🔹 Si l'utilisateur a saisi un nom → filtrage par nom
  if (query.isNotEmpty) {
    return achats.where((achat) {
      final nom = achat['nomAgriculteur']?.toLowerCase() ?? '';
      return nom.contains(query);
    }).toList();
  }

  // 🔹 Sinon → filtrage par date
  if (dateDebut != null && dateFin != null) {
    return achats.where((achat) {
      final date = DateTime.tryParse(achat['dateAchat']);
      return date != null &&
          date.isAfter(dateDebut!.subtract(Duration(days: 1))) &&
          date.isBefore(dateFin!.add(Duration(days: 1)));
    }).toList();
  }
...
Widget build(BuildContext context) {
  ...
  child: TextField(
    ...
    decoration: InputDecoration(
      labelText: 'Rechercher par planteur',
      prefixIcon: Icon(Icons.search),
      border: OutlineInputBorder(),
    ),
    onChanged: (value) {
    _onSearchAchats(value); // Appel de la méthode de recherche
  },
  ),
              ...
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500),
                ...
                Builder(
                  builder: (context) {
                    ...
                    return
            Text("Qté achetée: $sommeQuantite Kg", style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20));
                  },
                )
              ],
            ),
